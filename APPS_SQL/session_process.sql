Alter system kill session '2312,129';
...
-- tb.MODULE = 'ARXTWMAI'
--          tB.status = 'ACTIVE'
--tB.BLOCKING_SESSION =1421
--tB.ACTION like '%TNV%'
--tB.MODULE like '%JGZZVATSP%'
--and machine = 'uaperp2'
;

select * from v$sql where action like '%SELECT SUM%' ;
and module = 'APXPAWKB';

select substr(s1.SQL_TEXT,1,9999), s1.SQL_ID, s1.MODULE  from v$sql s1 where SQL_TEXT like 'insert%GL_JE_HEADER%';
