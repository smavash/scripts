...
ORAENV_FILE="/app/appl/XXX/inst/apps/XXX_server/ora/10.1.3/XXX_server.env"
...
if [ $control_code = "status" ]; then
               		$LINUX32 /app/appl/XXX/inst/apps/XXX_server/ora/10.1.3/opmn/bin/opmnctl $control_code -l -noheaders 2>&1 | tee -a $LOGAP  

	PROCSTATE=`$LINUX32 /app/appl/XXX/inst/apps/XXX_server/ora/10.1.3/opmn/bin/opmnctl $control_code -l -noheaders 2>&1 | tee -a $LOGAP|grep Alive|wc -l`
         exit_code=$?;
