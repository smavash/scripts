/*$Header: coe_xplain_80.sql 8.0 156959.1 2002/09/03            csierra coe $*/
SET term off ver off trims on;
/*=============================================================================

coe_xplain_80.sql - Enhanced Explain Plan For Given SQL Statement (8.0)
 ...
 --------
    ...
    coe_xplain_80.sql generates an Enhanced Explain Plan for one SQL statement
    to diagnose apps performance issues.

    It includes relevant statistics for tables, indexes and columns.

    It displays histograms, storage parameters and database parameters.
 ...
 1. Copy this whole Note into a text file.  Name it coe_xplain_80.sql when
    saving your text file.  Be sure filename is coe_xplain_80.sql.
 ...
 5. Place coe_xplain_80.sql and sql.txt flat files into same dedicated
    directory.

    coe_xplain_80.sql will try to open sql.txt under same directory.

 6. Execute coe_xplain_80.sql from SQL*Plus connected as user with access to
    all objects referenced by SQL statement in sql.txt.

    If you are using coe_xplain_80.sql within an Apps instance, connect as
    ...
    SQL> START coe_xplain_80.sql sql.txt;

 7. Review output files (spool): COE_XPLAIN_SQL.TXT and COE_STATEMENT_SQL.TXT

    The spool files get created on same directory from which coe_xplain_80.sql
 ...
 1. Always download latest version from Metalink (Note:156959.1), or ftp from
 ...
 5. This script has been tested up to Oracle Apps 11.0.2 with Oracle 8.0.5.
...
10. For Oracle 8.1 (8i) use coe_xplain.sql from Note:156958.1

    For Oracle 8.0 use coe_xplain_80.sql from Note:156959.1

    For Oracle 7.3 use coe_xplain_73.sql from Note:156960.1

11. For other SQL Tuning scripts, search on Metalink using keyword coescripts.

12. A practical guide in Troubleshooting Oracle ERP Applications Performance
 ...
 ----------
    ...
    coe_xplain_80.sql requires one user parameter to specify the name of the
    file containing the SQL statement to be explained.  This parameter is
    passed on the same command line used to execute coe_xplain_80.sql.
    # sqlplus apps/apps@vis11
    SQL> START coe_xplain_80.sql sql.txt;
    coe_xplain_80.sql includes also 3 fixed or seeded parameters.  They should
 ...
 ------
    ...
    Abstract: coe_xplain_80.sql - Enhanced Explain Plan For Given SQL Statement
    Author: Carlos Sierra
    Date: 03-SEP-02
    Description: Non-intrusive SQL Tuning tool to diagnose performance issues
    EMail: carlos.sierra@oracle.com
    Internal_Only: N
    Keywords: coe_xplain.sql explainplan sqltuning coescripts appsperf
    Metalink_Note: 156959.1
    New_Win: Y
    Product: SQL*Plus script
    Version: 8.0 2002/09/03
    Download: coe_xplain_80.zip
...
PROMPT
PROMPT ========================================================================
PROMPT coe_xplain_80.sql - Enhanced Explain Plan for given SQL statement (8.0)
PROMPT ========================================================================
PROMPT
...
CREATE TABLE COE_HISTOGRAM_COLS_XYZ
    (TABLE_NUM NUMBER,TABLE_NAME VARCHAR2(30),COLUMN_NAME VARCHAR2(30),
     PARTITION VARCHAR2(30),HSIZE NUMBER);
...
COLUMN query_plan FORMAT a140 HEADING -
       'Explain Plan by coe_xplain_80.sql 8.0 2002/09/03 Note:156959.1' wor;
...
COLUMN ended FORMAT A23 HEADING 'coe_xplain_80.sql ended';
...
SELECT
    'INSERT INTO COE_HISTOGRAMS_XYZ SELECT '||TABLE_NUM||', '''||
    TABLE_OWNER||''', '''||TABLE_NAME||''', COLUMN_NAME, ENDPOINT_NUMBER, '||
    'ENDPOINT_VALUE, NULL, NULL '||
...
UNION
SELECT             -- Columns with existing Histograms
...
SELECT
    ...
    ATP.AVG_SPACE,         -- Avg free space per allocated block in bytes
    ROUND(ATP.AVG_SPACE/TO_NUMBER(VP.VALUE)*100,3)
       AVG_SPACE_PERCENT   -- Avg free space per allocated block (percent)
FROM
    V$PARAMETER         VP,
    ALL_TAB_PARTITIONS  ATP,
...
PROMPT
PROMPT I.h TABLE Partition Extents
PROMPT ===========================
BREAK ON TABLE_NUM SKIP 2 -
      ON PARTITION_NAME SKIP 1 -
      ON TABLESPACE_NAME;
COMPUTE SUM LABEL "" OF BLOCKS BYTES ON TABLE_NUM PARTITION_NAME;
SELECT
    CT.TABLE_NUM,          -- Table Number
    DE.PARTITION_NAME,     -- Partition Name
    DE.TABLESPACE_NAME,    -- Tablespace
    DE.EXTENT_ID,          -- Extent ID
    DE.FILE_ID,            -- File ID
    DE.BLOCK_ID,           -- Block ID from
...
AND CT.TABLE_NAME   = DE.SEGMENT_NAME
AND CT.PARTITIONED  = 'YES'
AND DE.SEGMENT_TYPE = 'TABLE PARTITION'
ORDER BY
    CT.TABLE_NUM,
    DE.TABLESPACE_NAME,
    DE.EXTENT_ID;
...
PROMPT ===================================
BREAK ON TABLE_NUM ON INDEX_NUM SKIP 1;
SELECT
    CI.TABLE_NUM,         -- Table Number
    CI.INDEX_NUM,         -- Index Number
...
PROMPT ==========================================
BREAK ON TABLE_NUM ON INDEX_NUM SKIP 1;
COMPUTE SUM LABEL "" OF LEAF_BLOCKS CLUSTERING_FACTOR ON INDEX_NUM;
SELECT
    CI.TABLE_NUM,                -- Table Number
    CI.INDEX_NUM,                -- Index Number
    AIP.PARTITION_POSITION,      -- Partition Number
...
PROMPT
PROMPT II.j INDEX Partition Extents
PROMPT ============================
BREAK ON TABLE_NUM SKIP 3 -
      ON INDEX_NUM SKIP 2 -
      ON PARTITION_NAME SKIP 1 -
      ON TABLESPACE_NAME;
COMPUTE SUM LABEL "" OF BLOCKS BYTES ON TABLE_NUM INDEX_NUM PARTITION_NAME;
SELECT
    CI.TABLE_NUM,          -- Table Number
    CI.INDEX_NUM,          -- Index Number
    DE.PARTITION_NAME,     -- Partition Name
...
AND CI.INDEX_NAME   = DE.SEGMENT_NAME
AND CI.PARTITIONED  = 'YES'
AND DE.SEGMENT_TYPE = 'INDEX PARTITION'
ORDER BY
    CI.TABLE_NUM,
    CI.INDEX_NUM,
    DE.PARTITION_NAME,
...
PROMPT =======================================
BREAK ON TABLE_NUM ON INDEX_NUM ON PARTITION_POSITION SKIP 1;
SELECT
    CI.TABLE_NUM,           -- Table Number
    CI.INDEX_NUM,           -- Index Number
    ATP.PARTITION_POSITION, -- Partition Number
...
PROMPT =======================================
BREAK ON TABLE_NUM ON PARTITION_POSITION SKIP 1;
SELECT
    CT.TABLE_NUM,          -- Table Number
    ATP.PARTITION_POSITION,-- Partition Number
    ATC.COLUMN_ID,         -- Sequence number of column as created
...
PROMPT
PROMPT IV.a COLUMN HISTOGRAM candidates
PROMPT ================================
BREAK ON TABLE_NUM ON TABLE_NAME SKIP 1;
SELECT
...
/* XII. v$parameter and product component version Section                    */
/* ========================================================================= */
PROMPT
