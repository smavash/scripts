...
get_matrix() {
    ...
    #pmap -x $p_pid | tr '[]' '  ' | egrep -v "^ *Address|^---------|total Kb|^[0-9]*:" | gawk ''
    join -j 1 /tmp/procmm-x.$$ /tmp/procmm-s.$$  > /tmp/blah 
    join -j 1 /tmp/procmm-x.$$ /tmp/procmm-s.$$ | sed 's/ism shmid/ism_shmid/g' | tr '[]' '  ' | egrep -v "^ *Address|^---------|total Kb|^[0-9]*:" | gawk '
    {
        item=$7
        #print "before", s
        sub(/.*\.so.*/, "lib", item)
...
echo "\n-- procmm.sh: Process Memory Matrix "
...
if [ $compute_total -eq 0 ]; then 
    for pid in $pidlist; do
        # ps -opid,vsz,rss,args -p $pid | gawk '/ *[0-9]+/{ printf "-- ps info: PID=%d VSZ=%d RSS=%d ARGS=%s\n\n", $1,$2,$3,$4 }'
