...
BACKGROUND_DUMP_DEST=`strings ${SPFILE} ${INITORA} ${IFILE} | grep -i "user_dump_dest" 2>/dev/null |grep -v "^#" |head -1 |awk -F"=" '{print $2}' |sed -e 's/[       ][      ]*//g' -e "s/'//g"`
  if [ -z "${BACKGROUND_DUMP_DEST}" ]; then
    BACKGROUND_DUMP_DEST=${ORACLE_HOME}/admin/${ORACLE_SID}_uapdb1/diag/rdbms/prod/PROD/trace
  fi
...
do
  ...
  if($1=="Mon" || $1=="Tue" || $1=="Wed" || $1=="Thu" || $1=="Fri" || $1=="Sat" || $1=="Sun")
   ...
   { 
    print DATE"/"MONTH"/"YEAR" "DAY" "TIME" "$0 
   }
  }'
